  ...
  private async chat(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        const isRateLimit = err instanceof BackendError && err.statusCode === 429;
        if (isRateLimit && attempt < maxRetries) {
          const delayMs =
            process.env.NODE_ENV === 'test' ? 1 : baseDelayMs * 2 ** attempt + Math.random() * 200;
          emit({
            type: 'error',
            err: new Error(
              `Rate limit hit (429). Retrying in ${(delayMs / 1000).toFixed(1)}s (attempt ${attempt + 1}/${maxRetries})…`,
            ),
          });
